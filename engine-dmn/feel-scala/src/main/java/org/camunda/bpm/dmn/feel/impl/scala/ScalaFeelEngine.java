...
public class ScalaFeelEngine implements FeelEngine {
  ...
  public boolean evaluateSimpleUnaryTests(String expression,
    ...
    VariableProvider inputVariableContext =
      new JavaVariableProvider.StaticVariableProvider(Collections.singletonMap("inputVariableName", inputVariable));

    VariableProvider contextVariableWrapper = new ContextVariableWrapper(variableContext);

    CustomContext context = new CustomContext() {
      public VariableProvider variableProvider() {
        return new JavaVariableProvider.CompositeVariableProvider(Arrays.asList(inputVariableContext, contextVariableWrapper));
      }
    };
